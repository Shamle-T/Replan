...
interface PlanViewProps { tasks: Task[]; schedule: ScheduledTask[]; dayStart: Date; dayEnd: Date; onSaveTask: (task: Task) => void; onRemoveTask: (taskId: string) => void; onFindTime: (taskId: string) => void; onOptimize: () => void; notice?: string; }
...
export default function PlanView(props: PlanViewProps) {
  ...
  return <main className="plan-shell"><header className="plan-header"><div><p className="eyebrow">Constraint-aware planning</p><h1>Plan your day</h1><p>Fixed commitments stay anchored while flexible tasks can find their best legal time.</p></div><button className="primary-action" type="button" onClick={props.onOptimize}>Optimize day</button></header>{props.notice ? <p className="notice" role="status">{props.notice}</p> : null}<div className="plan-grid"><aside className="task-panel"><TaskForm task={editingTask} onSave={(task) => { props.onSaveTask(task); setEditingTaskId(null); }} onCancel={() => setEditingTaskId(null)} /><h2>Unscheduled tasks</h2>{unscheduled.length === 0 ? <p className="muted">Everything has a time.</p> : unscheduled.map((task) => <article className="task-card" key={task.id}><strong>{task.title}</strong><span>{categoryLabel(categoryForTask(task))} · {formatDurationCompact(task.durationMinutes)}</span>{task.deadline ? <small>Due {formatTime(task.deadline)}</small> : null}<div><button type="button" onClick={() => props.onFindTime(task.id)}>Find a time</button><button type="button" onClick={() => setEditingTaskId(task.id)}>Edit</button><button type="button" onClick={() => props.onRemoveTask(task.id)}>Remove</button></div></article>)}</aside><section className="schedule-panel"><h2>Today</h2><DayCalendar tasks={props.tasks} schedule={props.schedule} dayStart={props.dayStart} dayEnd={props.dayEnd} onTaskClick={setEditingTaskId} /></section></div></main>;
}
